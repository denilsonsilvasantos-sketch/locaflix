...
import { formatCurrency, cn, calculateMaxInstallments } from '../../lib/utils'
...
export function PropertyCard({ property, onFavoriteToggle, isFavorited = false, checkIn, checkOut, guests, pricePeriods = [] }: PropertyCardProps) {
  ...
  const total = hasDates && nights > 0 && checkIn && checkOut
    ? (() => {
        const estadiaResult = calcularEstadia(
        ...
        const combinedBase = estadiaResult.total + cleaning
        return combinedBase + Math.round(combinedBase * guestFeePercent * 100) / 100
      })()
    : 0
