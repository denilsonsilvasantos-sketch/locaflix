...
// ============================================================
// Endpoint de teste da integração Asaas (remover após validar)
// Acesso: GET /api/test/asaas?secret=<ASAAS_WEBHOOK_SECRET>&type=boleto&value=3
// ============================================================

app.get('/api/test/asaas', async (req: Request, res: Response) => {
  const { secret, type = 'boleto', value = '3' } = req.query as Record<string, string>
  ...
  if (!ASAAS_API_KEY) {
    res.status(503).json({ error: 'ASAAS_API_KEY não configurada.' })
    return
  }
  ...
  try {
    ...
    const payment = await asaasRequest('POST', '/payments', {
      ...
      value: amount,
      dueDate,
      description: `Teste integração Locaflix — ${billingType}`,
